...
# =========================
# 11. v1.5候補: Eloをλ補正に使ったまま、Eloだけ段階更新する検証
# =========================
...
# 比較対象として2つの前年設定を残します。
# 第一候補: PREV_WEIGHT=0.60, PREV_DECAY=0.985
# 比較候補: PREV_WEIGHT=0.40, PREV_DECAY=0.995
...
# Eloは従来通りλ補正に使う。
# 0.00はEloなし、0.10は直近のv1.5候補、0.20はv1.4系の基準に近い値。
ELO_LAMBDA_WEIGHT_CANDIDATES = [
    0.00, 0.05, 0.10, 0.15, 0.20, 0.25,
]

# シミュレーション内の仮想試合結果をEloへ反映する間隔。
# 0: 固定Elo, 1: 毎試合更新, 3: 3試合おき更新, 5: 5試合おき更新。
# λの攻守係数部分は更新せず、Eloだけを更新する。
ELO_UPDATE_EVERY_CANDIDATES = [0, 1, 3, 5]

OUTPUT_GRID_CSV = BASE_DIR / "v15_elo_lambda_dynamic_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v15_elo_lambda_dynamic_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v15_elo_lambda_dynamic_best_prediction.html"


def simulate_with_dynamic_elo_lambda(
    prev_setting,
    elo_lambda_weight,
    elo_update_every,
    n_sim,
    ...
    """
    Eloを従来通りλ補正に使いつつ、シミュレーション内の仮想試合結果でEloだけ段階更新する。

    固定点:
      ・λの基礎部分は、攻撃係数 × 守備係数 × 平均得点で作る
      ・攻守係数は後半戦中に更新しない
      ・Eloだけを仮想結果で更新し、次の試合以降のλ補正に反映する
      ・相性Effectは従来通りλに掛ける
    ...
    base_elo_ratings = prepared["elo_ratings"]
    ...
    for sim in range(1, n_sim + 1):
        ...
        # 各シミュレーションは、前半終了時点Eloから開始する。
        # elo_update_every=0なら固定Eloとして使う。
        sim_elo_ratings = dict(base_elo_ratings)
        pending_elo_updates = []
        ...
        for row in test_df.itertuples(index=False):
            ...
            # Eloをλに反映する。更新ありなら、その時点のsim_elo_ratingsを使う。
            lambda_home, lambda_away = expected_goals_home_away(
                ...
                elo_ratings=sim_elo_ratings if elo_lambda_weight > 0 else None,
            ...
            # λの基礎攻守係数は固定し、Eloだけを仮想結果で段階更新する。
            # elo_update_every=3なら、直近3試合分をまとめて反映し、次の試合以降に使う。
            if elo_update_every > 0 and elo_lambda_weight > 0:
        ...
        if verbose and sim % 1000 == 0:
            print(
                ...
                f"ELO_UPDATE_EVERY={elo_update_every}: {sim}/{n_sim} 回終了"
            )
    ...
    for team in teams:
        ...
        rows.append({
            ...
            "elo_initial": base_elo_ratings.get(team, INITIAL_ELO),
        })
...
def export_grid_html(df, output_path, title):
    ...
    round_cols = [
        ...
        "position_error", "elo_initial", "prev_weight_candidate", "prev_decay_candidate",
        "elo_lambda_weight_candidate", "elo_update_every_candidate", "effective_prev_weight"
    ]
    for col in round_cols:
        if col in display_df.columns:
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、Eloをλ補正に使ったまま、シミュレーション内でEloだけを段階更新する検証結果です。
      大勝補正={GOAL_ADJUST_MODE}, cap={GOAL_CAP_FOR_STRENGTH},
...
def prepare_common_data():
    ...
    base_elo_ratings = build_elo_ratings(
        previous_df=previous_df,
        train_df=train_df,
        teams=teams,
        k_factor=K_FACTOR,
...
def main():
    ...
    # elo_lambda_weight=0.00 の場合は更新しても結果が同じなので、update_every=0だけ実行する。
    combos = []
    for prev_setting in PREV_SETTING_CANDIDATES:
        for elo_lambda_weight in ELO_LAMBDA_WEIGHT_CANDIDATES:
            for elo_update_every in ELO_UPDATE_EVERY_CANDIDATES:
                if np.isclose(elo_lambda_weight, 0.0) and elo_update_every != 0:
                    continue
                combos.append((prev_setting, elo_lambda_weight, elo_update_every))

    total = len(combos)
    done = 0

    print("\n==============================")
    print("Elo λ補正 + Elo段階更新 グリッドサーチ開始")
    print("==============================")

    for prev_setting, elo_lambda_weight, elo_update_every in combos:
        done += 1
        print(
            f"\n[{done}/{total}] "
            f"{prev_setting['name']} "
        ...
        summary, prediction_df = simulate_with_dynamic_elo_lambda(
    ...
    # 比較用: 第一候補の固定Elo λ補正 ELO=0.10との差分
    baseline_rows = grid_df[
        (grid_df["prev_setting_name"] == "prev060_decay0985")
        & np.isclose(grid_df["elo_lambda_weight"], 0.10)
        & (grid_df["elo_update_every"] == 0)
    ]
    if len(baseline_rows) > 0:
        baseline_mae = float(baseline_rows.iloc[0]["mae"])
        grid_df["delta_vs_prev060_decay0985_elo010_fixed"] = grid_df["mae"] - baseline_mae
    else:
        grid_df["delta_vs_prev060_decay0985_elo010_fixed"] = np.nan

    # 比較用: v1.4寄りのElo=0.20固定との差分
    baseline2_rows = grid_df[
        (grid_df["prev_setting_name"] == "prev060_decay0985")
        & np.isclose(grid_df["elo_lambda_weight"], 0.20)
        & (grid_df["elo_update_every"] == 0)
    ]
    if len(baseline2_rows) > 0:
        baseline2_mae = float(baseline2_rows.iloc[0]["mae"])
        grid_df["delta_vs_prev060_decay0985_elo020_fixed"] = grid_df["mae"] - baseline2_mae
    else:
        grid_df["delta_vs_prev060_decay0985_elo020_fixed"] = np.nan
    ...
    grid_df["rank_within_update_every"] = (
    ...
    show_cols = [
        ...
        "elo_lambda_weight", "elo_update_every", "mae",
        "delta_vs_prev060_decay0985_elo010_fixed",
        "delta_vs_prev060_decay0985_elo020_fixed",
    ...
    print(grid_df[show_cols].head(20).to_string(index=False))
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        best_elo_lambda_weight = float(best_summary["elo_lambda_weight"])
        ...
        print("best ELO_LAMBDA_WEIGHT:", best_elo_lambda_weight)
        print("best ELO_UPDATE_EVERY:", best_elo_update_every)

        final_summary, final_prediction = simulate_with_dynamic_elo_lambda(
            prev_setting=best_prev_setting,
            elo_lambda_weight=best_elo_lambda_weight,
        ...
        export_grid_html(
            ...
            OUTPUT_BEST_HTML,
            title=(
                "J1 2025 順位予測 v1.5候補 Elo λ補正 動的更新 "
                f"{best_prev_setting_name}, ELO={best_elo_lambda_weight:.2f}, update={best_elo_update_every}"
            ),
        )
        ...
        print("ELO_LAMBDA_WEIGHT:", best_elo_lambda_weight)
        ...
        print(final_prediction[[
            ...
            "elo_mode", "elo_lambda_weight_candidate", "elo_update_every_candidate",
            "effective_prev_weight", "champion_prob", "top3_prob", "bottom3_prob",
            "avg_points", "avg_gd", "elo_initial",
