# NOTE: 優勝圧力を「順位ベース＋勝点差サブ補正」に変更した検証版です。
import pandas as pd
import numpy as np
...
def calculate_pressure_context(table, played, total_games_by_team, teams):
    ...
    for team in teams:
        ...
        # 優勝圧力。
        # 従来は「首位との勝点差」を主軸にしていたが、
        # この版では「順位」を主軸にし、勝点差はサブ補正として使う。
        title_distance = max(0.0, top_points - team_points)

        if USE_TITLE_RANK_BASED_PRESSURE:
            # 1位=1.00, 2位=0.85, ... のように、順位で基本圧力を決める。
            rank_base_pressure = TITLE_RANK_BASE_PRESSURE.get(team_rank, 0.0)

            # 勝点差はサブ補正。
            # 首位に近いほど1.0、TITLE_POINTS_WINDOW以上離れると下限値まで下がる。
            raw_points_factor = max(0.0, 1.0 - title_distance / TITLE_POINTS_WINDOW)
            points_sub_factor = (
                TITLE_POINTS_SUB_FACTOR_FLOOR
                + (1.0 - TITLE_POINTS_SUB_FACTOR_FLOOR) * raw_points_factor
            )

            title_pressure = rank_base_pressure * points_sub_factor
        else:
            # 従来版: 首位との勝点差を主軸にする。
            ...
            title_pressure *= upper_table_gate
...
def apply_pressure_effects_to_lambdas(lambda_home, lambda_away, home, away, pressure_context):
    ...
    優勝争い:
      ・TITLE_ATTACK_LAMBDA_BOOST > 0 なら、優勝争いチームの得点λを上げる
      ・TITLE_DEFENSE_LAMBDA_BOOST > 0 なら、優勝争いチームと対戦する相手の得点λを下げる
      ・LEADER_ATTACK_LAMBDA_DEBUFF > 0 なら、首位だけ得点λを下げる
      ・CHASER_ATTACK_LAMBDA_BOOST > 0 なら、2〜TITLE_CHASER_MAX_RANK位だけ得点λを上げる
      ・CHASER_DEFENSE_LAMBDA_BOOST > 0 なら、追走側と対戦する相手の得点λを下げる
    """
    if not USE_PRESSURE_EFFECTS:
        return safe_lambda(lambda_home), safe_lambda(lambda_away), {
            ...
            "home_leader_pressure": 0.0,
            "away_leader_pressure": 0.0,
            "home_chaser_pressure": 0.0,
            "away_chaser_pressure": 0.0,
        }
    ...
    home_rank = int(home_ctx.get("rank", 999))
    away_rank = int(away_ctx.get("rank", 999))

    home_leader = home_title if home_rank == 1 else 0.0
    away_leader = away_title if away_rank == 1 else 0.0

    home_chaser = home_title if 2 <= home_rank <= TITLE_CHASER_MAX_RANK else 0.0
    away_chaser = away_title if 2 <= away_rank <= TITLE_CHASER_MAX_RANK else 0.0

    home_multiplier = np.exp(
        RELEGATION_ATTACK_LAMBDA_BOOST * home_rel
        - RELEGATION_DEFENSE_LAMBDA_BOOST * away_rel
        + TITLE_ATTACK_LAMBDA_BOOST * home_title
        - TITLE_ATTACK_LAMBDA_DEBUFF * home_title
        - TITLE_DEFENSE_LAMBDA_BOOST * away_title
        - LEADER_ATTACK_LAMBDA_DEBUFF * home_leader
        + CHASER_ATTACK_LAMBDA_BOOST * home_chaser
        - CHASER_DEFENSE_LAMBDA_BOOST * away_chaser
    )
    away_multiplier = np.exp(
        RELEGATION_ATTACK_LAMBDA_BOOST * away_rel
        - RELEGATION_DEFENSE_LAMBDA_BOOST * home_rel
        + TITLE_ATTACK_LAMBDA_BOOST * away_title
        - TITLE_ATTACK_LAMBDA_DEBUFF * away_title
        - TITLE_DEFENSE_LAMBDA_BOOST * home_title
        - LEADER_ATTACK_LAMBDA_DEBUFF * away_leader
        + CHASER_ATTACK_LAMBDA_BOOST * away_chaser
        - CHASER_DEFENSE_LAMBDA_BOOST * home_chaser
    )
    ...
    pressure_info = {
        ...
        "home_leader_pressure": home_leader,
        "away_leader_pressure": away_leader,
        "home_chaser_pressure": home_chaser,
        "away_chaser_pressure": away_chaser,
    }
...
# PREV設定は 0.40 / 0.995、Elo補正重みは0.20に固定する。
PREV_SETTING_FIXED = {"name": "prev040_decay0995", "prev_weight": 0.40, "prev_decay": 0.995}
...
OUTPUT_GRID_CSV = BASE_DIR / "v15_prev040_decay0995_elo020_title_boost_rank_based_tuning_results.csv"
OUTPUT_GRID_HTML = BASE_DIR / "v15_prev040_decay0995_elo020_title_boost_rank_based_tuning_results.html"
OUTPUT_BEST_CSV = BASE_DIR / "v15_prev040_decay0995_elo020_title_boost_rank_based_tuning_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v15_prev040_decay0995_elo020_title_boost_rank_based_tuning_best_prediction.html"
...
# 優勝圧力の計算方法
# True : 順位を主軸にし、首位との勝点差はサブ補正として使う
# False: 従来通り、首位との勝点差を主軸にする
USE_TITLE_RANK_BASED_PRESSURE = True

# 順位ごとの基本優勝圧力。1位ほど強く、5位までを優勝争い圏として扱う。
TITLE_RANK_BASE_PRESSURE = {
    1: 1.00,
    2: 0.85,
    3: 0.70,
    4: 0.55,
...
# 勝点差サブ補正の下限。
# 例: 0.30なら、首位から6点以上離れていても順位圧力の30%は残る。
TITLE_POINTS_SUB_FACTOR_FLOOR = 0.30
...
# 優勝争い補正: 一律ブースト/デバフ
TITLE_ATTACK_LAMBDA_BOOST = 0.00
TITLE_ATTACK_LAMBDA_DEBUFF = 0.00
TITLE_DEFENSE_LAMBDA_BOOST = 0.00

# 優勝争い補正: 首位と追走側を分ける設定
TITLE_CHASER_MAX_RANK = 4
LEADER_ATTACK_LAMBDA_DEBUFF = 0.00
CHASER_ATTACK_LAMBDA_BOOST = 0.00
CHASER_DEFENSE_LAMBDA_BOOST = 0.00
...
def make_pressure_setting(
    ...
    return {
        ...
        "title_attack_lambda_boost": title_attack_lambda_boost,
        "title_attack_lambda_debuff": title_attack_lambda_debuff,
        "title_defense_lambda_boost": title_defense_lambda_boost,
        "title_chaser_max_rank": title_chaser_max_rank,
        "leader_attack_lambda_debuff": leader_attack_lambda_debuff,
        "chaser_attack_lambda_boost": chaser_attack_lambda_boost,
        "chaser_defense_lambda_boost": chaser_defense_lambda_boost,
...
# 優勝ブースト検証用候補。
# PREV=0.40/0.995, ELO=0.20 は固定し、終盤補正だけを比較する。
PRESSURE_SETTING_CANDIDATES = [
    make_pressure_setting(
        "pressure_off",
    ...
    make_pressure_setting(
        "title_boost_weak",
        title_attack_lambda_boost=0.03,
        title_defense_lambda_boost=0.02,
        max_draw_shift=0.00,
    ),
    make_pressure_setting(
        "title_boost_mid",
        title_attack_lambda_boost=0.05,
        title_defense_lambda_boost=0.03,
        max_draw_shift=0.00,
    ),
    make_pressure_setting(
        "title_boost_strong",
        title_attack_lambda_boost=0.07,
        title_defense_lambda_boost=0.04,
        max_draw_shift=0.00,
    ),
    make_pressure_setting(
        "title_boost_mid_late070",
    ...
    # 首位は少し硬くなるが、追走側は勢いが出る仮説
    make_pressure_setting(
        "leader_debuff_chaser_boost_weak",
        leader_attack_lambda_debuff=0.03,
        chaser_attack_lambda_boost=0.03,
        chaser_defense_lambda_boost=0.02,
        draw_shift_title=0.010,
        max_draw_shift=0.04,
    ),
    make_pressure_setting(
        "leader_debuff_chaser_boost_mid",
        leader_attack_lambda_debuff=0.04,
        chaser_attack_lambda_boost=0.04,
        chaser_defense_lambda_boost=0.03,
        draw_shift_title=0.015,
        max_draw_shift=0.05,
    ),
    make_pressure_setting(
        "leader_debuff_chaser_boost_late070",
        late_season_start_ratio=0.70,
        leader_attack_lambda_debuff=0.04,
        chaser_attack_lambda_boost=0.04,
        chaser_defense_lambda_boost=0.03,
    ...
    # 残留ブーストと優勝ブーストを併用する仮説
    make_pressure_setting(
        "relegation_weak_title_boost_weak",
    ...
    make_pressure_setting(
        "relegation_mid_title_boost_weak",
    ...
    make_pressure_setting(
        "relegation_mid_title_boost_mid",
        relegation_attack_lambda_boost=0.04,
        relegation_defense_lambda_boost=0.04,
        title_attack_lambda_boost=0.05,
        title_defense_lambda_boost=0.03,
    ...
    # 残留ブースト + 首位デバフ/追走ブースト
    make_pressure_setting(
        "relegation_mid_leader_chaser_mid",
        relegation_attack_lambda_boost=0.04,
        relegation_defense_lambda_boost=0.04,
        leader_attack_lambda_debuff=0.04,
        chaser_attack_lambda_boost=0.04,
...
def apply_pressure_setting(setting):
    ...
    global TITLE_ATTACK_LAMBDA_BOOST
    global TITLE_ATTACK_LAMBDA_DEBUFF
    global TITLE_DEFENSE_LAMBDA_BOOST
    global TITLE_CHASER_MAX_RANK
    global LEADER_ATTACK_LAMBDA_DEBUFF
    global CHASER_ATTACK_LAMBDA_BOOST
    global CHASER_DEFENSE_LAMBDA_BOOST
    global DRAW_SHIFT_RELEGATION
    global DRAW_SHIFT_TITLE
    global MAX_DRAW_SHIFT

    PRESSURE_SETTING_NAME = setting["name"]
    USE_PRESSURE_EFFECTS = bool(setting.get("use_pressure_effects", True))
    LATE_SEASON_START_RATIO = float(setting.get("late_season_start_ratio", 0.65))
    RELEGATION_POINTS_WINDOW = float(setting.get("relegation_points_window", 6.0))
    TITLE_POINTS_WINDOW = float(setting.get("title_points_window", 6.0))
    RELEGATION_ATTACK_LAMBDA_BOOST = float(setting.get("relegation_attack_lambda_boost", 0.0))
    RELEGATION_DEFENSE_LAMBDA_BOOST = float(setting.get("relegation_defense_lambda_boost", 0.0))
    TITLE_ATTACK_LAMBDA_BOOST = float(setting.get("title_attack_lambda_boost", 0.0))
    TITLE_ATTACK_LAMBDA_DEBUFF = float(setting.get("title_attack_lambda_debuff", 0.0))
    TITLE_DEFENSE_LAMBDA_BOOST = float(setting.get("title_defense_lambda_boost", 0.0))
    TITLE_CHASER_MAX_RANK = int(setting.get("title_chaser_max_rank", 4))
    LEADER_ATTACK_LAMBDA_DEBUFF = float(setting.get("leader_attack_lambda_debuff", 0.0))
    CHASER_ATTACK_LAMBDA_BOOST = float(setting.get("chaser_attack_lambda_boost", 0.0))
    CHASER_DEFENSE_LAMBDA_BOOST = float(setting.get("chaser_defense_lambda_boost", 0.0))
    DRAW_SHIFT_RELEGATION = float(setting.get("draw_shift_relegation", 0.0))
    DRAW_SHIFT_TITLE = float(setting.get("draw_shift_title", 0.0))
    MAX_DRAW_SHIFT = float(setting.get("max_draw_shift", 0.0))


def export_pressure_grid_html(df, output_path, title):
    """優勝ブースト検証結果の一覧HTMLを出力する。"""
    display_df = df.copy()
    ...
    round_cols = [
        ...
        "title_attack_lambda_boost", "title_attack_lambda_debuff", "title_defense_lambda_boost",
        "leader_attack_lambda_debuff", "chaser_attack_lambda_boost", "chaser_defense_lambda_boost",
        "draw_shift_relegation", "draw_shift_title", "max_draw_shift",
        "kawasaki_error", "kyoto_error", "kashiwa_error",
    ...
    column_names = {
        ...
        "use_title_rank_based_pressure": "優勝順位ベース",
        "title_points_sub_factor_floor": "勝点差係数下限",
        "title_chaser_max_rank": "追走上限順位",
        "relegation_attack_lambda_boost": "残留攻撃λ+",
        "relegation_defense_lambda_boost": "残留守備λ-",
        "title_attack_lambda_boost": "優勝攻撃λ+",
        "title_attack_lambda_debuff": "優勝攻撃λ-",
        "title_defense_lambda_boost": "優勝守備λ-",
        "leader_attack_lambda_debuff": "首位攻撃λ-",
        "chaser_attack_lambda_boost": "追走攻撃λ+",
        "chaser_defense_lambda_boost": "追走守備λ-",
...
<body>
  ...
  <div class="note">
    <p>
      PREV_WEIGHT=0.40, PREV_DECAY=0.995, ELO_LAMBDA_WEIGHT=0.20 は固定。
      優勝ブーストを「順位ベース＋勝点差サブ補正」で計算し、首位デバフ＋追走ブースト、残留ブースト併用も比較しています。
    </p>
    <p>
...
def simulate_with_prev_and_elo(
    ...
    summary = {
        ...
        "relegation_points_window": RELEGATION_POINTS_WINDOW,
        "title_points_window": TITLE_POINTS_WINDOW,
        "use_title_rank_based_pressure": USE_TITLE_RANK_BASED_PRESSURE,
        "title_points_sub_factor_floor": TITLE_POINTS_SUB_FACTOR_FLOOR,
        "relegation_attack_lambda_boost": RELEGATION_ATTACK_LAMBDA_BOOST,
        "relegation_defense_lambda_boost": RELEGATION_DEFENSE_LAMBDA_BOOST,
        "title_attack_lambda_boost": TITLE_ATTACK_LAMBDA_BOOST,
        "title_attack_lambda_debuff": TITLE_ATTACK_LAMBDA_DEBUFF,
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、PREV=0.40/0.995、Elo補正重み=0.20を固定し、
...
def prepare_common_data():
    ...
    print("USE_TITLE_RANK_BASED_PRESSURE:", USE_TITLE_RANK_BASED_PRESSURE)
    print("TITLE_RANK_BASE_PRESSURE:", TITLE_RANK_BASE_PRESSURE)
    print("TITLE_POINTS_SUB_FACTOR_FLOOR:", TITLE_POINTS_SUB_FACTOR_FLOOR)
...
def main():
    ...
    print("\n==============================")
    print("優勝ブースト順位ベース版・首位デバフ/追走ブースト チューニング開始")
    print("==============================")
    ...
    for i, pressure_setting in enumerate(PRESSURE_SETTING_CANDIDATES, start=1):
        ...
        print(
            ...
            f"TITLE_BOOST=({TITLE_ATTACK_LAMBDA_BOOST:.3f}, def={TITLE_DEFENSE_LAMBDA_BOOST:.3f}), "
            f"LEADER/CHASER=({LEADER_ATTACK_LAMBDA_DEBUFF:.3f}, {CHASER_ATTACK_LAMBDA_BOOST:.3f}, {CHASER_DEFENSE_LAMBDA_BOOST:.3f}), "
            f"draw_title={DRAW_SHIFT_TITLE:.3f}, late={LATE_SEASON_START_RATIO:.2f}, "
            f"window=({RELEGATION_POINTS_WINDOW:.1f}, {TITLE_POINTS_WINDOW:.1f})"
        )
    ...
    export_pressure_grid_html(
        grid_df,
        OUTPUT_GRID_HTML,
        title="J1 2025 優勝ブースト補正 順位ベース版 チューニング結果",
    )
    ...
    show_cols = [
        ...
        "late_season_start_ratio", "relegation_points_window", "title_points_window",
        "use_title_rank_based_pressure", "title_points_sub_factor_floor",
        "relegation_attack_lambda_boost", "relegation_defense_lambda_boost",
        "title_attack_lambda_boost", "title_attack_lambda_debuff", "title_defense_lambda_boost",
        "leader_attack_lambda_debuff", "chaser_attack_lambda_boost", "chaser_defense_lambda_boost",
        "draw_shift_relegation", "draw_shift_title",
    ...
    if RUN_FINAL_BEST and best_summary is not None and best_setting is not None:
        ...
        print("RELEGATION_ATTACK_LAMBDA_BOOST:", RELEGATION_ATTACK_LAMBDA_BOOST)
        print("RELEGATION_DEFENSE_LAMBDA_BOOST:", RELEGATION_DEFENSE_LAMBDA_BOOST)
        print("TITLE_ATTACK_LAMBDA_BOOST:", TITLE_ATTACK_LAMBDA_BOOST)
        print("TITLE_ATTACK_LAMBDA_DEBUFF:", TITLE_ATTACK_LAMBDA_DEBUFF)
        print("TITLE_DEFENSE_LAMBDA_BOOST:", TITLE_DEFENSE_LAMBDA_BOOST)
        print("USE_TITLE_RANK_BASED_PRESSURE:", USE_TITLE_RANK_BASED_PRESSURE)
        print("TITLE_RANK_BASE_PRESSURE:", TITLE_RANK_BASE_PRESSURE)
        print("TITLE_POINTS_SUB_FACTOR_FLOOR:", TITLE_POINTS_SUB_FACTOR_FLOOR)
        print("LEADER_ATTACK_LAMBDA_DEBUFF:", LEADER_ATTACK_LAMBDA_DEBUFF)
        print("CHASER_ATTACK_LAMBDA_BOOST:", CHASER_ATTACK_LAMBDA_BOOST)
        print("CHASER_DEFENSE_LAMBDA_BOOST:", CHASER_DEFENSE_LAMBDA_BOOST)
        print("DRAW_SHIFT_RELEGATION:", DRAW_SHIFT_RELEGATION)
        print("DRAW_SHIFT_TITLE:", DRAW_SHIFT_TITLE)
        ...
        export_grid_html(
            ...
            OUTPUT_BEST_HTML,
            title=(
                "J1 2025 順位予測 v1.5 優勝ブースト補正 最良候補 "
