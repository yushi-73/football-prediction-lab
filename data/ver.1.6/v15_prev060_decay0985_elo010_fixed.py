...
# PREV設定は 0.60 / 0.985 に固定する。
# ここでは前年レーティング設定を固定したうえで、Elo補正の強さだけを検証する。
PREV_SETTING_CANDIDATES = [
    {"name": "prev060_decay0985", "prev_weight": 0.60, "prev_decay": 0.985},
]

# Elo補正重みは 0.10 に固定する。
# PREV_WEIGHT=0.60, PREV_DECAY=0.985, ELO_LAMBDA_WEIGHT=0.10 の単一モデル。
ELO_LAMBDA_WEIGHT_CANDIDATES = [
    0.10,
]

OUTPUT_GRID_CSV = BASE_DIR / "v15_prev060_decay0985_elo010_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v15_prev060_decay0985_elo010_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v15_prev060_decay0985_elo010_prediction.html"
...
def main():
    ...
    print("\n==============================")
    print("PREV=0.60/0.985・Elo補正0.10 固定モデル開始")
    print("==============================")
    ...
    # 単一モデルなので、比較用delta列は固定モデル自身との差分として0にする。
    grid_df["delta_vs_prev060_decay0985_elo010"] = 0.0
    ...
    show_cols = [
        ...
        "mae", "delta_vs_prev060_decay0985_elo010",
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        print("fixed ELO_LAMBDA_WEIGHT:", best_elo_lambda_weight)
        ...
        export_grid_html(
            ...
            OUTPUT_BEST_HTML,
            title=(
                "J1 2025 順位予測 v1.5 prev060_decay0985・ELO010固定 "
                f"ELO={best_elo_lambda_weight:.2f}"
            ),
        )
