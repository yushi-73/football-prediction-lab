...
#   manager_boost_counterfactual_j1_2005_2025_summary_v15.csv
#   manager_boost_counterfactual_j1_2005_2025_by_event_v15.csv
#   manager_boost_counterfactual_j1_2005_2025_by_year_v15.csv
#   manager_boost_counterfactual_j1_2005_2025_unmatched_v15.csv
#   manager_boost_counterfactual_j1_2005_2025_summary_v15.html
# ============================================================
...
EVENTS_CSV_CANDIDATES = [
    ...
    "manager_events_j1_2005_2025_collapsed_for_boost.csv",
    "manager_events_j1_for_boost_v2.csv",
...
# 出力
OUTPUT_MATCHES_CSV = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_matches_v15.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_summary_v15.csv"
OUTPUT_BY_EVENT_CSV = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_by_event_v15.csv"
OUTPUT_BY_YEAR_CSV = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_by_year_v15.csv"
OUTPUT_UNMATCHED_CSV = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_unmatched_v15.csv"
OUTPUT_HTML = BASE_DIR / "manager_boost_counterfactual_j1_2005_2025_summary_v15.html"
...
def load_events(path):
    ...
    # 2005〜2025版のイベントCSVには、すでに event_id が入っている。
    # その状態で insert すると "cannot insert event_id, already exists" が出るため、
    # 既存の event_id があればそれを優先して使う。
    if "event_id" in events.columns:
        events["event_id"] = events["event_id"].astype(str).str.strip()
        missing_event_id = events["event_id"].isin(["", "nan", "None"]) | events["event_id"].isna()
        if missing_event_id.any():
            events.loc[missing_event_id, "event_id"] = [
                f"E{i:03d}" for i in range(1, int(missing_event_id.sum()) + 1)
            ]
    else:
        events.insert(0, "event_id", [f"E{i:03d}" for i in range(1, len(events) + 1)])

    # 同一チーム・同一シーズンの次の監督交代日を付与する。
    # 入力CSVに next_change_date がある場合も、現在の並びから再計算して上書きする。
