...
N_SIM_SEARCH = 10000
RANDOM_SEED = 42
LAMBDA_CAP = 3.5  # main内でLAMBDA_CAP_CANDIDATESに応じて切り替える

# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "lambda_cap_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "lambda_cap_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "lambda_cap_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "lambda_cap_simple_multiyear_best_predictions.csv"
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        ...
        "max_match_draw_prob", "lambda_cap", "matchup_time_decay", "mean_mae", "std_mae",
        "mean_prob_actual_position", "mean_sim_draw_rate",
        "mae_2023", "mae_2024", "mae_2025",
        "delta_vs_lambda_cap35",
    ]
...
<body>
  <h1>Lambda Cap 3年比較</h1>
  <div class="note">
    <p>
      2022→2023、2023→2024、2024→2025を対象に、PREV_WEIGHT=0.20 / PREV_DECAY=0.995、
      ELO_LAMBDA_WEIGHT=0.20、大勝補正cap4、COMPAT_WEIGHT=0.20、DRAW_FACTOR=1.20、CURRENT_DECAY=1.000を固定し、λキャップを比較しています。
    </p>
    <p>
...
def main():
    ...
    print("LAMBDA_CAP_CANDIDATES:", LAMBDA_CAP_CANDIDATES)
    ...
    total = len(LAMBDA_CAP_CANDIDATES) * len(TARGET_YEARS)
    done = 0
    ...
    global LAMBDA_CAP
    for lambda_cap_candidate in LAMBDA_CAP_CANDIDATES:
        LAMBDA_CAP = lambda_cap_candidate
        lambda_cap_label = "none" if LAMBDA_CAP is None else f"cap{float(LAMBDA_CAP):g}"
        ...
        for target_year in TARGET_YEARS:
            ...
            lambda_cap_seed = 0 if lambda_cap_candidate is None else int(round(float(lambda_cap_candidate) * 1000))
            seed_offset = (
            ...
            prediction_df["lambda_cap"] = np.nan if lambda_cap_candidate is None else float(lambda_cap_candidate)
    ...
    pivot_mae = detail_df.pivot_table(
        index=[
            ...
            "compat_weight", "current_decay", "shrinkage", "draw_factor", "max_match_draw_prob_label", "lambda_cap_label", "max_goals_for_score_grid",
    ...
    # 比較基準: LAMBDA_CAP=3.5、つまり従来よく使っていたλ上限
    base_rows = summary_df[summary_df["lambda_cap_label"].astype(str) == "cap3.5"]
    base_lambda_cap35 = float(base_rows.iloc[0]["mean_mae"]) if len(base_rows) > 0 else np.nan
    summary_df["delta_vs_lambda_cap35"] = summary_df["mean_mae"] - base_lambda_cap35
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "current_decay", "shrinkage", "draw_factor", "max_match_draw_prob_label", "lambda_cap_label", "max_goals_for_score_grid",
        "matchup_prior_n", "matchup_time_decay",
