...
# ============================================================
# J1順位予測 ver.1.5 候補: 前年レーティング最適化グリッドサーチ
# ------------------------------------------------------------
# 目的:
#   jleague_team_stats_yearly_wide_2019_2025.csv を使い、
#   前年スタッツのどの重みが順位予測に効くかを検証する。
#
# 方針:
#   ・v1.4の大勝補正 cap4 は維持
#   ・2025前半など当年前半の係数は従来通り「得点 + 枠内シュート」
#   ・前年係数に、公式チームスタッツから作った攻撃/守備係数を弱く混ぜる
#   ・昇格組も前年J2/J3スタッツがあれば、リーグ補正をかけて弱く反映できる
#
# 使い方:
#   python grid_search_v15_prev_stats.py
#
# 出力:
#   v15_prev_stats_grid_results.csv
#   v15_prev_stats_best_prediction.csv
#   v15_prev_stats_best_prediction.html
# ============================================================
...
# 検証対象年。
# 現在は2025だけで試し、j1_2021...などの試合CSVがそろったら複数年に拡張できます。
TARGET_YEARS = [2025]
...
# 前年の試合別データで使う特徴量
# 2024以前の試合別データには枠内/xGがない前提なので、従来通り総シュート10%
...
# 昇格組補正
# 前年J1試合がないチームでも、前年公式スタッツがあれば前年重みを使う。
# ただし、PREV_STATS_WEIGHT=0の基準モデルではv1.4に合わせて前年重み0にする。
USE_PREV_STATS_FOR_PROMOTED = True
PROMOTED_PREV_WEIGHT_WITH_STATS = 0.4
PROMOTED_PREV_WEIGHT_NO_STATS = 0.0

# リーグ補正: J2/J3の前年スタッツをJ1基準へ近づけるために1.0へ縮小する
LEAGUE_SHRINK = {
    "j1": 1.00,
    "j2": 0.60,
    "j3": 0.45,
...
# シミュレーション
# 探索中は1000程度で十分。最終候補だけ10000で確認する。
N_SIM_SEARCH = 1000
N_SIM_FINAL = 10000
RUN_FINAL_BEST = True
DECAY = 1.0
...
# 出力
OUTPUT_GRID_CSV = BASE_DIR / "v15_prev_stats_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v15_prev_stats_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v15_prev_stats_best_prediction.html"
...
# 既存の前年係数に公式スタッツ係数をどれだけ混ぜるか
# 0.0はv1.4基準の比較用。
PREV_STATS_WEIGHT_CANDIDATES = [0.0, 0.05, 0.10, 0.15, 0.20]
...
def build_prev_stats_strengths(team_stats_df, target_year, teams, attack_weights, defense_weights):
...
def apply_prev_stats_to_previous_strengths(previous_strengths, prev_stats_strengths, teams, prev_stats_weight):
    adjusted = {}

    for team in teams:
        base = previous_strengths.get(team, {
            "home_attack": 1.0,
            "home_defense": 1.0,
            "away_attack": 1.0,
            "away_defense": 1.0,
        })
        stats = prev_stats_strengths.get(team)

        if stats is None or prev_stats_weight <= 0:
            adjusted[team] = base.copy()
        else:
            adjusted[team] = {
                key: (1.0 - prev_stats_weight) * base[key] + prev_stats_weight * stats[key]
...
def blend_with_current_strengths(current_strengths, previous_strengths, previous_df, teams, prev_stats_meta, prev_stats_weight):
    blended = {}
    prev_weight_by_team = {}
    prev_games_by_team = {}
    ...
    for team in teams:
        ...
        has_prev_stats = team in prev_stats_meta

        if prev_games > 0:
            effective_prev_weight = PREV_WEIGHT
        elif USE_PREV_STATS_FOR_PROMOTED and has_prev_stats and prev_stats_weight > 0:
            effective_prev_weight = PROMOTED_PREV_WEIGHT_WITH_STATS
        else:
            effective_prev_weight = PROMOTED_PREV_WEIGHT_NO_STATS
...
def evaluate_config(context, team_stats_df, attack_weights, defense_weights, prev_stats_weight, n_sim, seed, return_prediction=False):
    target_year = context["target_year"]
    teams = context["teams"]

    prev_stats_strengths, prev_stats_meta = build_prev_stats_strengths(
    ...
    previous_strengths = apply_prev_stats_to_previous_strengths(
        previous_strengths=context["previous_strengths_base"],
        prev_stats_strengths=prev_stats_strengths,
        teams=teams,
        prev_stats_weight=prev_stats_weight,
    )

    strengths, prev_weight_by_team, prev_games_by_team = blend_with_current_strengths(
    ...
    # 対戦ごとのlambdaは設定ごとに固定なので先に計算しておく
    fixtures = []
...
def export_prediction_html(df, output_path, title="J1 順位予測 ver.1.5 前年スタッツ最適化"):
    display_df = df.copy()
    ...
    round_cols = [
        ...
        "prev_stats_attack_ratio", "prev_stats_defense_bad_ratio", "effective_prev_weight", "prev_stats_weight",
    ]
...
<body>
  ...
  <div class="note">
    <p>
      v1.4の大勝補正cap4を維持し、前年公式スタッツによる前年レーティング補正をグリッドサーチしました。
...
def main():
    ...
    print("\n==============================")
    print("ver.1.5 前年スタッツ最適化グリッドサーチ")
    print("==============================")
    ...
    configs = []
    for prev_stats_weight, attack_weights, defense_weights in product(
        PREV_STATS_WEIGHT_CANDIDATES,
        ATTACK_WEIGHT_CANDIDATES,
        DEFENSE_WEIGHT_CANDIDATES,
    ):
        # prev_stats_weight=0では攻守重みが結果に影響しないので、最初の1件だけ残す
        if prev_stats_weight == 0.0:
            if attack_weights != ATTACK_WEIGHT_CANDIDATES[0] or defense_weights != DEFENSE_WEIGHT_CANDIDATES[0]:
                continue
        configs.append((prev_stats_weight, attack_weights, defense_weights))

    print("config数:", len(configs))

    grid_rows = []
    for idx, (prev_stats_weight, attack_weights, defense_weights) in enumerate(configs, start=1):
        year_maes = []
        for ctx in contexts:
            seed = RANDOM_SEED + ctx["target_year"] * 1000
            mae, _ = evaluate_config(
                context=ctx,
                team_stats_df=team_stats_df,
                attack_weights=attack_weights,
                defense_weights=defense_weights,
    ...
    if RUN_FINAL_BEST:
        ...
        best_prev_stats_weight = float(best["prev_stats_weight"])

        all_pred = []
        final_maes = []
        for ctx in contexts:
            seed = RANDOM_SEED + ctx["target_year"] * 1000
            mae, pred_df = evaluate_config(
                context=ctx,
                team_stats_df=team_stats_df,
                attack_weights=best_attack,
                defense_weights=best_defense,
                prev_stats_weight=best_prev_stats_weight,
                n_sim=N_SIM_FINAL,
                seed=seed,
        ...
        print("\n==============================")
        print("最良設定をN_SIM_FINALで再評価")
        print("==============================")
        print("best config_id:", int(best["config_id"]))
        print("prev_stats_weight:", best_prev_stats_weight)
