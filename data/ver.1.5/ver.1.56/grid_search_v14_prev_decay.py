...
# 今回はPREV_WEIGHTはv1.4既定値の0.40で固定し、
# 前年レーティングを作るときだけPREV_DECAYを変えます。
FIXED_PREV_WEIGHT = PREV_WEIGHT

# 1.00は前年全試合を均等評価。値を下げるほど前年終盤を重視します。
PREV_DECAY_CANDIDATES = [
    1.000, 0.995, 0.990, 0.985, 0.980, 0.975, 0.970,
]

OUTPUT_GRID_CSV = BASE_DIR / "v14_prev_decay_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v14_prev_decay_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v14_prev_decay_best_prediction.html"


def simulate_with_prev_decay(
    prev_decay,
    n_sim,
    prepared,
    seed=42,
    ...
    """
    v1.4構造を保ったまま、PREV_WEIGHTは固定し、前年レーティング計算用のPREV_DECAYだけを差し替えて順位予測する。
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    for sim in range(1, n_sim + 1):
        ...
        if verbose and sim % 1000 == 0:
            print(f"  prev_decay={prev_decay:.3f}: {sim}/{n_sim} 回終了")
    ...
    for team in teams:
        ...
        rows.append({
            ...
            "prev_weight_candidate": FIXED_PREV_WEIGHT,
    ...
    # 川崎Fなど、診断でよく見るチームの値をsummaryへ載せる
...
def export_grid_html(df, output_path, title):
    ...
    column_names = {
        ...
        "avg_pred_position": "平均予測順位",
        "position_error": "順位誤差",
        "prev_weight_candidate": "固定PREV_WEIGHT",
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、PREV_WEIGHTは固定し、前年レーティング計算時だけPREV_DECAYを変えた結果です。
      大勝補正={GOAL_ADJUST_MODE}, cap={GOAL_CAP_FOR_STRENGTH},
...
def prepare_common_data():
    ...
    print("FIXED_PREV_WEIGHT:", FIXED_PREV_WEIGHT)
...
def main():
    ...
    print("\n==============================")
    print("PREV_DECAY グリッドサーチ開始")
    print("==============================")

    for i, prev_decay in enumerate(PREV_DECAY_CANDIDATES, start=1):
        print(f"\n[{i}/{len(PREV_DECAY_CANDIDATES)}] PREV_DECAY={prev_decay:.3f}")
        summary, prediction_df = simulate_with_prev_decay(
            prev_decay=prev_decay,
            n_sim=N_SIM_SEARCH,
            prepared=prepared,
            seed=RANDOM_SEED,
    ...
    # v1.4既定値PREV_DECAY=1.000との差分列を追加
    baseline_rows = grid_df[np.isclose(grid_df["prev_decay"], 1.0)]
    if len(baseline_rows) > 0:
        baseline_mae = float(baseline_rows.iloc[0]["mae"])
        grid_df["delta_vs_decay100"] = grid_df["mae"] - baseline_mae
    else:
        grid_df["delta_vs_decay100"] = np.nan
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        print("==============================")
        print("fixed PREV_WEIGHT:", FIXED_PREV_WEIGHT)
        print("best PREV_DECAY:", best_prev_decay)

        final_summary, final_prediction = simulate_with_prev_decay(
        ...
        export_grid_html(
            final_prediction,
            OUTPUT_BEST_HTML,
            title=f"J1 2025 順位予測 v1.4 PREV_DECAY検証 best={best_prev_decay:.3f}",
        )

        print("\n==============================")
        print("最終確認")
        print("==============================")
        print("PREV_WEIGHT固定:", FIXED_PREV_WEIGHT)
