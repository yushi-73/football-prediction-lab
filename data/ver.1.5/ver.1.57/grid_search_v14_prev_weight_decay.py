...
# =========================
# 11. PREV_WEIGHT × PREV_DECAY 同時検証用設定
# =========================
...
# 今回はver.1.4をベースに、前年を何%混ぜるかと、前年終盤をどれくらい重く見るかを同時に試します。
PREV_WEIGHT_CANDIDATES = [
    0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60,
]

# 1.000は前年全試合を均等評価。値を下げるほど前年終盤を重視します。
# 前回の単独検証で強いdecayは悪化気味だったので、今回は0.980までに絞っています。
PREV_DECAY_CANDIDATES = [
    1.000, 0.995, 0.990, 0.985, 0.980,
]

OUTPUT_GRID_CSV = BASE_DIR / "v14_prev_weight_decay_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v14_prev_weight_decay_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v14_prev_weight_decay_best_prediction.html"


def simulate_with_prev_weight_decay(
    prev_weight,
    prev_decay,
    n_sim,
    prepared,
    ...
    """
    v1.4構造を保ったまま、PREV_WEIGHTと前年レーティング計算用PREV_DECAYを同時に差し替えて順位予測する。

    変更点:
      ・最終攻守係数 = 2025年前半係数 × (1 - prev_weight) + 2024前年係数 × prev_weight
      ・2024前年係数を作るときだけ prev_decay を使う

    固定点:
      ・2025年前半係数はDECAY=1.0のまま
      ・Elo補正、相性Effect、大勝補正cap4などはv1.4設定を維持
...
def export_grid_html(df, output_path, title):
    ...
    round_cols = [
        ...
        "position_error", "elo", "prev_weight_candidate", "prev_decay_candidate", "effective_prev_weight"
    ]
    for col in round_cols:
        if col in display_df.columns:
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、PREV_WEIGHTと前年レーティング計算時のPREV_DECAYを同時探索した結果です。
      大勝補正={GOAL_ADJUST_MODE}, cap={GOAL_CAP_FOR_STRENGTH},
      2025枠内重み={SOT_WEIGHT}, 2024総シュート重み={PREV_SHOT_WEIGHT},
      Elo補正={ELO_LAMBDA_WEIGHT}, 相性Effect={"ON" if USE_MATCHUP_EFFECT else "OFF"}。
    </p>
  </div>
  <div class="table-wrap">
    {table_html}
...
def main():
    ...
    total = len(PREV_WEIGHT_CANDIDATES) * len(PREV_DECAY_CANDIDATES)
    done = 0

    print("\n==============================")
    print("PREV_WEIGHT × PREV_DECAY グリッドサーチ開始")
    print("==============================")
    ...
    for prev_weight in PREV_WEIGHT_CANDIDATES:
        for prev_decay in PREV_DECAY_CANDIDATES:
            ...
            summary, prediction_df = simulate_with_prev_weight_decay(
                prev_weight=prev_weight,
                prev_decay=prev_decay,
                n_sim=N_SIM_SEARCH,
                prepared=prepared,
    ...
    # v1.4既定値 PREV_WEIGHT=0.40, PREV_DECAY=1.000 との差分列を追加
    baseline_rows = grid_df[
        np.isclose(grid_df["prev_weight"], 0.40) & np.isclose(grid_df["prev_decay"], 1.0)
    ]
    if len(baseline_rows) > 0:
        baseline_mae = float(baseline_rows.iloc[0]["mae"])
        grid_df["delta_vs_v14_prev04_decay100"] = grid_df["mae"] - baseline_mae
    else:
        grid_df["delta_vs_v14_prev04_decay100"] = np.nan

    # 各PREV_WEIGHT内の順位もあると、decayの効き方を見やすい
    grid_df["rank_within_prev_weight"] = (
        grid_df.groupby("prev_weight")["mae"]
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        final_summary, final_prediction = simulate_with_prev_weight_decay(
        ...
        export_grid_html(
            ...
            title=(
                "J1 2025 順位予測 v1.4 "
                f"PREV_WEIGHT×PREV_DECAY検証 best={best_prev_weight:.2f}, {best_prev_decay:.3f}"
            ),
        )
        ...
        print(final_prediction[[
            ...
            "position_error", "prev_weight_candidate", "prev_decay_candidate", "effective_prev_weight",
