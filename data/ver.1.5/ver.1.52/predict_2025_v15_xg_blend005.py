...
# ============================================================
# J1 2025 順位予測 ver.1.5 xGブレンド検証版
# ------------------------------------------------------------
# 採用モデル:
#   ・2025前半戦: 得点 + 枠内シュート
#   ・2024前年: 得点 + 総シュート
...
#
# ver.1.5 追加点:
#   ・xGを独立特徴量として足すのではなく、攻守係数計算用の得点をxG側へ少し寄せる
#   ・2025前半戦: 攻守係数用得点 = 95%補正後実得点 + 5%xG
#   ・2024以前はxGデータがないため、xGブレンドは自動的にOFFになり、従来通り「総シュート10%」のまま
#   ・枠内シュート重みはv1.4と同じ10%を維持する
# ============================================================
...
OUTPUT_TAG = f"v15_{GOAL_ADJUST_MODE}_xgblend005"
OUTPUT_CSV = BASE_DIR / f"j1_2025_prediction_{OUTPUT_TAG}.csv"
OUTPUT_HTML = BASE_DIR / f"j1_2025_prediction_{OUTPUT_TAG}.html"

# 基本モデル
# ver.1.5では、枠内シュート重みはv1.4と同じ10%のまま維持する。
# xGは独立した特徴量ではなく、攻守係数計算用の得点を少しならすために使う。
SOT_WEIGHT = 0.1
XG_WEIGHT = 0.0
...
# xGブレンド
# True : 攻守係数計算用の得点を xG 側へ少し寄せる
# False: v1.4と同じく、得点capのみを使う
USE_XG_GOAL_BLEND = True
XG_GOAL_BLEND_WEIGHT = 0.05
CLIP_XG_BLEND_RESULT_TO_GOAL_CAP = True
...
def add_goal_for_strength_columns(df):
    ...
    v1.4:
        大勝補正として、攻守係数計算用の得点をcapする。

    v1.5 xGブレンド:
        xGが存在するデータでは、補正後実得点を少しだけxG側へ寄せる。
        例: XG_GOAL_BLEND_WEIGHT = 0.05 の場合
            home_goal_strength = 0.95 * cap後の実得点 + 0.05 * home_xg

    注意:
        ・順位表計算、実順位、初期勝点には生の home_goal / away_goal を使う。
        ・この列は、チームの攻撃力・守備力を推定するときだけ使う。
        ・2024以前のようにxG列がない場合は、自動的にv1.4と同じ挙動になる。
    """
    df = df.copy()

    if GOAL_ADJUST_MODE == "raw":
        home_base = df["home_goal"].astype(float)
        away_base = df["away_goal"].astype(float)

    elif GOAL_ADJUST_MODE in ["cap3", "cap4"]:
        cap = float(GOAL_CAP_FOR_STRENGTH)
        home_base = df["home_goal"].astype(float).clip(upper=cap)
        away_base = df["away_goal"].astype(float).clip(upper=cap)

    else:
        raise ValueError("GOAL_ADJUST_MODE は 'raw', 'cap3', 'cap4' のどれかにしてください。")

    df["home_goal_strength"] = home_base.astype(float)
    df["away_goal_strength"] = away_base.astype(float)

    # xGが使えるデータだけ、攻守係数用得点を少しだけxG側へ寄せる。
    # 2024以前などxGが存在しないデータでは何もしない。
    has_xg_cols = "home_xg" in df.columns and "away_xg" in df.columns
    has_xg_values = (
        has_xg_cols
        and df["home_xg"].notna().any()
        and df["away_xg"].notna().any()
    )

    if USE_XG_GOAL_BLEND and XG_GOAL_BLEND_WEIGHT > 0 and has_xg_values:
        blend = float(XG_GOAL_BLEND_WEIGHT)

        home_xg = pd.to_numeric(df["home_xg"], errors="coerce").fillna(home_base)
        away_xg = pd.to_numeric(df["away_xg"], errors="coerce").fillna(away_base)

        df["home_goal_strength"] = (1 - blend) * home_base + blend * home_xg
        df["away_goal_strength"] = (1 - blend) * away_base + blend * away_xg

        # cap4の外れ値耐性を維持したい場合は、ブレンド後も上限を超えないようにする。
        if CLIP_XG_BLEND_RESULT_TO_GOAL_CAP and GOAL_ADJUST_MODE in ["cap3", "cap4"]:
            cap = float(GOAL_CAP_FOR_STRENGTH)
            df["home_goal_strength"] = df["home_goal_strength"].clip(upper=cap)
            df["away_goal_strength"] = df["away_goal_strength"].clip(upper=cap)
...
def calculate_strengths_home_away(
    ...
    feature_weight:
        枠内シュートまたは総シュートに割り当てる重み。
    xg_weight:
        xG / 被xG に割り当てる重み。

    ver.1.5の基本形:
        2025前半戦: 得点90% + 枠内シュート5% + xG5%
        2024前年   : 得点90% + 総シュート10%
    """
    ...
    if feature_type == "sot":
        ...
        use_shot_feature = feature_weight > 0
        use_xg = False
        xg_weight = 0.0
    ...
    if use_xg:
        ...
        if not has_xg_values:
            print("警告: xGが指定されましたが、このデータではxG列が使えないため xg_weight=0 にします。")
            use_xg = False
            xg_weight = 0.0

    total_feature_weight = feature_weight + xg_weight
    if total_feature_weight < 0 or total_feature_weight >= 1.0:
        raise ValueError("feature_weight + xg_weight は 0以上1未満にしてください。")

    goal_weight = 1.0 - total_feature_weight
    ...
    for team in teams:
        ...
        strengths[team] = {
            "home_attack": safe_strength(
                goal_weight * home_goal_attack
                + feature_weight * home_shot_attack
                + xg_weight * home_xg_attack
            ),
            "home_defense": safe_strength(
                goal_weight * home_goal_defense
                + feature_weight * home_shot_defense
                + xg_weight * home_xg_defense
            ),
            "away_attack": safe_strength(
                goal_weight * away_goal_attack
                + feature_weight * away_shot_attack
                + xg_weight * away_xg_attack
            ),
            "away_defense": safe_strength(
                goal_weight * away_goal_defense
                + feature_weight * away_shot_defense
                + xg_weight * away_xg_defense
            ),
        }
...
<head>
  <meta charset="UTF-8">
  <title>J1 2025 順位予測 | ver.1.5 xGブレンドテスト</title>
  <style>
    body {{
...
<body>
  <h1>J1 2025 順位予測 ver.1.5 xGブレンドテスト</h1>
  <div class="note">
    <p>
      採用設定: 前年レーティング重み {PREV_WEIGHT} /
      2025枠内シュート重み {SOT_WEIGHT} /
      2025xG特徴量重み {XG_WEIGHT} /
      xGブレンド {"ON" if USE_XG_GOAL_BLEND else "OFF"}, weight={XG_GOAL_BLEND_WEIGHT} /
    ...
    <p>
      ...
      2025前半戦は、攻守係数計算用の得点を「補正後実得点95% + xG5%」に少しだけ寄せたうえで、枠内シュート10%を加味しています。2024前年はxGなしのため、従来通り「得点90% + 総シュート10%」で攻守係数を計算しています。
...
def main():
    ...
    print("SOT_WEIGHT:", SOT_WEIGHT)
    print("XG_WEIGHT:", XG_WEIGHT)
    print("USE_XG_GOAL_BLEND:", USE_XG_GOAL_BLEND)
    print("XG_GOAL_BLEND_WEIGHT:", XG_GOAL_BLEND_WEIGHT)
    print("CLIP_XG_BLEND_RESULT_TO_GOAL_CAP:", CLIP_XG_BLEND_RESULT_TO_GOAL_CAP)
    ...
    for team in teams:
        ...
        rows.append({
            ...
            "xg_weight": XG_WEIGHT,
            "use_xg_goal_blend": USE_XG_GOAL_BLEND,
            "xg_goal_blend_weight": XG_GOAL_BLEND_WEIGHT,
            "clip_xg_blend_result_to_goal_cap": CLIP_XG_BLEND_RESULT_TO_GOAL_CAP,
    ...
    print(result_df[[
        ...
        "xg_weight",
        "use_xg_goal_blend",
        "xg_goal_blend_weight",
        "clip_xg_blend_result_to_goal_cap",
        "champion_prob",
        "top3_prob",
        "bottom3_prob",
        "avg_points",
