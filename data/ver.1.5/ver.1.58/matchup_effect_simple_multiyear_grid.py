...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "matchup_effect_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "matchup_effect_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "matchup_effect_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "matchup_effect_simple_multiyear_best_predictions.csv"
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        ...
        "delta_vs_compat000", "delta_vs_compat010", "delta_vs_compat020", "delta_vs_compat030",
    ]
...
<body>
  ...
  <div class="note">
    <p>
      ...
      ELO_LAMBDA_WEIGHT=0.20、大勝補正cap4を固定し、相性Effectの重みを比較しています。
    </p>
    <p>
...
def main():
    ...
    print("COMPAT_WEIGHT_CANDIDATES:", COMPAT_WEIGHT_CANDIDATES)
    ...
    total = len(COMPAT_WEIGHT_CANDIDATES) * len(TARGET_YEARS)
    done = 0

    for compat_weight in COMPAT_WEIGHT_CANDIDATES:
        for target_year in TARGET_YEARS:
            ...
            seed_offset = int(round(compat_weight * 1000))
    ...
    pivot_mae = detail_df.pivot_table(
        index=[
            ...
            "compat_weight", "matchup_prior_n", "matchup_time_decay",
    ...
    # 比較基準
    def baseline_mean_mae(compat_weight):
        rows = summary_df[np.isclose(summary_df["compat_weight"], compat_weight)]
        if len(rows) == 0:
            return np.nan
        return float(rows.iloc[0]["mean_mae"])

    base_000 = baseline_mean_mae(0.00)
    base_010 = baseline_mean_mae(0.10)
    base_020 = baseline_mean_mae(0.20)
    base_030 = baseline_mean_mae(0.30)

    summary_df["delta_vs_compat000"] = summary_df["mean_mae"] - base_000
    summary_df["delta_vs_compat010"] = summary_df["mean_mae"] - base_010
    summary_df["delta_vs_compat020"] = summary_df["mean_mae"] - base_020
    summary_df["delta_vs_compat030"] = summary_df["mean_mae"] - base_030
    ...
    best_compat_weight = float(summary_df.iloc[0]["compat_weight"])

    predictions_df = pd.concat(all_predictions, ignore_index=True)
    best_predictions_df = predictions_df[np.isclose(predictions_df["compat_weight"], best_compat_weight)].copy()
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "matchup_prior_n", "matchup_time_decay",
    ...
    print("COMPAT_WEIGHT:", best_compat_weight)
