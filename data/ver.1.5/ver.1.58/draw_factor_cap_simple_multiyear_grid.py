...
#   5. previous_year + target_year前半から固定Eloを作る
#   6. cutoff_date以前の全J1履歴から相性Effectを作る
#   7. COMPAT_WEIGHT=0.20を固定する
#   8. Draw Factorで引き分けスコアの確率を補正し、必要に応じて引き分け確率上限を適用する
#   9. target_year最終順位に対するMAEを計算する
# ============================================================
...
DRAW_FACTOR_CANDIDATES = [
    1.20,
]

# Draw Factor適用後の「1試合ごとの引き分け確率」に上限をつける候補。
# None は上限なし、つまり従来の DRAW_FACTOR=1.20 と同じ。
MAX_MATCH_DRAW_PROB_CANDIDATES = [
    None,
    0.33,
]
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "draw_factor_cap_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "draw_factor_cap_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "draw_factor_cap_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "draw_factor_cap_simple_multiyear_best_predictions.csv"
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        "prev_weight", "prev_decay", "elo_lambda_weight", "compat_weight", "draw_factor",
        ...
        "delta_vs_draw120_none", "delta_vs_cap030", "delta_vs_cap031", "delta_vs_cap032", "delta_vs_cap033",
    ]
...
<head>
  <meta charset="UTF-8">
  <title>Draw Factor 上限つき 3年比較</title>
  <style>
    body {{
...
<body>
  <h1>Draw Factor 上限つき 3年比較</h1>
  <div class="note">
    <p>
      2022→2023、2023→2024、2024→2025を対象に、PREV_WEIGHT=0.20 / PREV_DECAY=0.995、
      ELO_LAMBDA_WEIGHT=0.20、大勝補正cap4、COMPAT_WEIGHT=0.20、DRAW_FACTOR=1.20を固定し、引き分け確率上限を比較しています。
    </p>
    <p>
...
def main():
    ...
    print("\n==============================")
    print("Draw Factor 上限つき 3年比較 簡易版")
    print("==============================")
    ...
    print("DRAW_FACTOR_CANDIDATES:", DRAW_FACTOR_CANDIDATES)
    print("MAX_MATCH_DRAW_PROB_CANDIDATES:", MAX_MATCH_DRAW_PROB_CANDIDATES)
    ...
    total = len(DRAW_FACTOR_CANDIDATES) * len(MAX_MATCH_DRAW_PROB_CANDIDATES) * len(TARGET_YEARS)
    done = 0

    for draw_factor in DRAW_FACTOR_CANDIDATES:
        for max_match_draw_prob in MAX_MATCH_DRAW_PROB_CANDIDATES:
            ...
            for target_year in TARGET_YEARS:
                ...
                seed_offset = int(round(draw_factor * 1000)) + cap_seed
    ...
    pivot_mae = detail_df.pivot_table(
        index=[
            ...
            "compat_weight", "draw_factor", "max_match_draw_prob_label", "max_goals_for_score_grid",
    ...
    # 比較基準
    def baseline_mean_mae(cap_label):
        rows = summary_df[summary_df["max_match_draw_prob_label"] == cap_label]
        if len(rows) == 0:
            return np.nan
        return float(rows.iloc[0]["mean_mae"])

    base_none = baseline_mean_mae("none")
    base_030 = baseline_mean_mae("cap030")
    base_031 = baseline_mean_mae("cap031")
    base_032 = baseline_mean_mae("cap032")
    base_033 = baseline_mean_mae("cap033")

    summary_df["delta_vs_draw120_none"] = summary_df["mean_mae"] - base_none
    summary_df["delta_vs_cap030"] = summary_df["mean_mae"] - base_030
    summary_df["delta_vs_cap031"] = summary_df["mean_mae"] - base_031
    summary_df["delta_vs_cap032"] = summary_df["mean_mae"] - base_032
    summary_df["delta_vs_cap033"] = summary_df["mean_mae"] - base_033
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "draw_factor", "max_match_draw_prob_label", "max_goals_for_score_grid",
        "matchup_prior_n", "matchup_time_decay",
