...
# Draw Factor の比較候補
# 1.00 が通常の独立ポアソン。
# 1.00より大きいほど 0-0, 1-1, 2-2 などの引き分けスコアが出やすくなる。
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "draw_factor_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "draw_factor_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "draw_factor_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "draw_factor_simple_multiyear_best_predictions.csv"
...
def sample_score_with_draw_factor(lambda_home, lambda_away, draw_factor, max_goals=MAX_GOALS_FOR_SCORE_GRID):
    """
    独立ポアソンのスコア確率行列を作り、引き分けスコアだけdraw_factor倍する。

    draw_factor=1.00なら通常の独立ポアソン。
    draw_factor>1.00なら引き分けスコアが増え、
    draw_factor<1.00なら引き分けスコアが減る。
    ...
    if draw_factor != 1.0:
        ...
        score_matrix[diag_idx, diag_idx] *= draw_factor
    ...
    flat_probs = (score_matrix / total_prob).ravel()
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        ...
        "delta_vs_draw090", "delta_vs_draw100", "delta_vs_draw110", "delta_vs_draw120",
    ]
...
<body>
  <h1>Draw Factor 3年比較</h1>
  <div class="note">
    <p>
      2022→2023、2023→2024、2024→2025を対象に、PREV_WEIGHT=0.20 / PREV_DECAY=0.995、
      ELO_LAMBDA_WEIGHT=0.20、大勝補正cap4、COMPAT_WEIGHT=0.20を固定し、Draw Factorを比較しています。
    </p>
    <p>
...
def main():
    ...
    total = len(DRAW_FACTOR_CANDIDATES) * len(TARGET_YEARS)
    done = 0
    ...
    for draw_factor in DRAW_FACTOR_CANDIDATES:
        for target_year in TARGET_YEARS:
            ...
            seed_offset = int(round(draw_factor * 1000))
    ...
    pivot_mae = detail_df.pivot_table(
        index=[
            ...
            "compat_weight", "draw_factor", "max_goals_for_score_grid",
    ...
    # 比較基準
    def baseline_mean_mae(draw_factor):
        rows = summary_df[np.isclose(summary_df["draw_factor"], draw_factor)]
        if len(rows) == 0:
            return np.nan
        return float(rows.iloc[0]["mean_mae"])

    base_090 = baseline_mean_mae(0.90)
    base_100 = baseline_mean_mae(1.00)
    base_110 = baseline_mean_mae(1.10)
    base_120 = baseline_mean_mae(1.20)

    summary_df["delta_vs_draw090"] = summary_df["mean_mae"] - base_090
    summary_df["delta_vs_draw100"] = summary_df["mean_mae"] - base_100
    summary_df["delta_vs_draw110"] = summary_df["mean_mae"] - base_110
    summary_df["delta_vs_draw120"] = summary_df["mean_mae"] - base_120
    ...
    best_predictions_df = predictions_df[np.isclose(predictions_df["draw_factor"], best_draw_factor)].copy()
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "draw_factor", "max_goals_for_score_grid",
        "matchup_prior_n", "matchup_time_decay",
