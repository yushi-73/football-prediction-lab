...
#   ・Elo補正の強さ ELO_LAMBDA_WEIGHT を複数年で比較する
...
#   6. Elo差をλに反映し、target_year最終順位に対するMAEを計算する
# ============================================================
...
# Elo補正の比較候補
# 0.00はElo補正なしの基準。まずは弱〜中程度の範囲を確認する。
...
# Eloをλに反映するときのスケール
# 既存v1.4系と同じ式: factor = 10 ** ((elo_weight * elo_diff) / ELO_FACTOR_SCALE)
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "elo_lambda_weight_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "elo_lambda_weight_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "elo_lambda_weight_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "elo_lambda_weight_simple_multiyear_best_predictions.csv"
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        "prev_weight", "prev_decay", "elo_lambda_weight", "mean_mae", "std_mae",
        "mean_prob_actual_position", "mean_sim_draw_rate",
        "mae_2023", "mae_2024", "mae_2025",
        "delta_vs_elo000", "delta_vs_elo010", "delta_vs_elo020",
    ]
...
<body>
  ...
  <div class="note">
    <p>
      2022→2023、2023→2024、2024→2025を対象に、PREV_WEIGHT=0.20 / PREV_DECAY=0.995 を固定し、
      Elo補正重みだけを比較しています。λは得点のみで作成し、シュート数・枠内シュート数・相性Effect・終盤補正は使っていません。
    </p>
    <p>
...
def main():
    ...
    total = len(ELO_LAMBDA_WEIGHT_CANDIDATES) * len(TARGET_YEARS)
    done = 0
    ...
    for elo_lambda_weight in ELO_LAMBDA_WEIGHT_CANDIDATES:
        for target_year in TARGET_YEARS:
            ...
            summary, prediction_df = simulate_target_year(
                ...
                prev_weight=prev_weight,
                prev_decay=prev_decay,
                elo_lambda_weight=float(elo_lambda_weight),
                n_sim=N_SIM_SEARCH,
                seed=RANDOM_SEED + target_year + int(round(float(elo_lambda_weight) * 1000)),
            )
            ...
            prediction_df["elo_lambda_weight"] = float(elo_lambda_weight)
    ...
    pivot_mae = detail_df.pivot_table(
        index=["prev_setting_name", "prev_weight", "prev_decay", "elo_lambda_weight"],
        columns="target_year",
        values="mae",
        aggfunc="mean",
    )
    ...
    summary_df = grouped.merge(
        pivot_mae,
        on=["prev_setting_name", "prev_weight", "prev_decay", "elo_lambda_weight"],
        how="left",
    )

    # 比較基準: Eloなし / 0.10 / 0.20
    def baseline_mean_mae(elo_weight):
        rows = summary_df[np.isclose(summary_df["elo_lambda_weight"], elo_weight)]
        if len(rows) == 0:
            return np.nan
        return float(rows.iloc[0]["mean_mae"])

    base_elo000 = baseline_mean_mae(0.00)
    base_elo010 = baseline_mean_mae(0.10)
    base_elo020 = baseline_mean_mae(0.20)

    summary_df["delta_vs_elo000"] = summary_df["mean_mae"] - base_elo000
    summary_df["delta_vs_elo010"] = summary_df["mean_mae"] - base_elo010
    summary_df["delta_vs_elo020"] = summary_df["mean_mae"] - base_elo020
    ...
    best_elo = float(summary_df.iloc[0]["elo_lambda_weight"])

    predictions_df = pd.concat(all_predictions, ignore_index=True)
    best_predictions_df = predictions_df[np.isclose(predictions_df["elo_lambda_weight"], best_elo)].copy()
    ...
    print("ELO_LAMBDA_WEIGHT:", best_elo)
