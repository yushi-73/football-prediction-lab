...
#   6. 混合後の攻守係数を 1.0 + (strength - 1.0) * SHRINKAGE で平均方向に戻す
...
# 攻守係数のShrinkage候補。
# 1.00 = 縮小なし。
# 0.90 = 攻守係数の平均との差を10%弱める。
# 例: 1.40 -> 1.36, 0.70 -> 0.73
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "shrinkage_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "shrinkage_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "shrinkage_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "shrinkage_simple_multiyear_best_predictions.csv"
...
def simulate_target_year(
    ...
    # Shrinkage: 混合後の攻守係数を平均1.0方向に少し戻す。
    # 前半戦の上振れ・下振れを抑える目的。
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        "prev_weight", "prev_decay", "elo_lambda_weight", "compat_weight", "shrinkage", "draw_factor",
...
def main():
    ...
    print("SHRINKAGE_CANDIDATES:", SHRINKAGE_CANDIDATES)
    ...
    total = len(SHRINKAGE_CANDIDATES) * len(TARGET_YEARS)
    done = 0
    ...
    for shrinkage in SHRINKAGE_CANDIDATES:
        for target_year in TARGET_YEARS:
            ...
            seed_offset = int(round(draw_factor * 1000)) + cap_seed + int(round(float(shrinkage) * 1000))
    ...
    pivot_mae = detail_df.pivot_table(
        index=[
            ...
            "compat_weight", "shrinkage", "draw_factor", "max_match_draw_prob_label", "max_goals_for_score_grid",
    ...
    # 比較基準: shrinkage=1.00、つまり縮小なし
    base_rows = summary_df[np.isclose(summary_df["shrinkage"].astype(float), 1.00)]
    base_shrink100 = float(base_rows.iloc[0]["mean_mae"]) if len(base_rows) > 0 else np.nan
    summary_df["delta_vs_shrink100"] = summary_df["mean_mae"] - base_shrink100
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "shrinkage", "draw_factor", "max_match_draw_prob_label", "max_goals_for_score_grid",
        "matchup_prior_n", "matchup_time_decay",
