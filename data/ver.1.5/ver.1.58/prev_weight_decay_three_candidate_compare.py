...
# ============================================================
# J1 前年レーティング係数 3候補比較用 簡易版
# ------------------------------------------------------------
# 目的:
#   ・前年係数 PREV_WEIGHT / PREV_DECAY の有力3候補を複数年で比較する
...
# 前年レーティング係数の比較候補
# 1. 前年なし
# 2. 前回グリッド最良: PREV_WEIGHT=0.20, PREV_DECAY=0.995
# 3. decayなし寄り比較: PREV_WEIGHT=0.20, PREV_DECAY=1.000
PREV_SETTING_CANDIDATES = [
    {"name": "prev000", "prev_weight": 0.00, "prev_decay": 1.000},
    {"name": "prev020_decay0995", "prev_weight": 0.20, "prev_decay": 0.995},
    {"name": "prev020_decay1000", "prev_weight": 0.20, "prev_decay": 1.000},
]

# 表示用。実際のループは PREV_SETTING_CANDIDATES を使う。
PREV_WEIGHT_CANDIDATES = sorted({s["prev_weight"] for s in PREV_SETTING_CANDIDATES})
PREV_DECAY_CANDIDATES = sorted({s["prev_decay"] for s in PREV_SETTING_CANDIDATES}, reverse=True)
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "prev_weight_decay_three_candidate_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "prev_weight_decay_three_candidate_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "prev_weight_decay_three_candidate_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "prev_weight_decay_three_candidate_best_predictions.csv"
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        ...
        "delta_vs_prev000", "delta_vs_prev020_decay0995", "delta_vs_prev020_decay1000",
    ]
...
<body>
  <h1>前年レーティング係数 3候補比較</h1>
  <div class="note">
    <p>
      2022→2023、2023→2024、2024→2025を対象に、前年なし・0.20/0.995・0.20/1.000の3候補を比較しています。
...
def main():
    ...
    total = len(PREV_SETTING_CANDIDATES) * len(TARGET_YEARS)
    done = 0

    for prev_setting in PREV_SETTING_CANDIDATES:
        prev_name = str(prev_setting["name"])
    ...
    pivot_mae = detail_df.pivot_table(
        index=["prev_setting_name", "prev_weight", "prev_decay"],
        columns="target_year",
        values="mae",
        aggfunc="mean",
    )
    ...
    grouped = detail_df.groupby(["prev_setting_name", "prev_weight", "prev_decay"], as_index=False).agg(
        mean_mae=("mae", "mean"),
        std_mae=("mae", "std"),
    ...
    summary_df = grouped.merge(pivot_mae, on=["prev_setting_name", "prev_weight", "prev_decay"], how="left")

    # 比較基準: 前年なし / 0.20-0.995 / 0.20-1.000
    ...
    base_020_0995 = baseline_mean_mae(0.20, 0.995)
    base_020_1000 = baseline_mean_mae(0.20, 1.000)

    summary_df["delta_vs_prev000"] = summary_df["mean_mae"] - base_prev000
    summary_df["delta_vs_prev020_decay0995"] = summary_df["mean_mae"] - base_020_0995
    summary_df["delta_vs_prev020_decay1000"] = summary_df["mean_mae"] - base_020_1000
    ...
    print("==============================")
    show_cols = [
        "rank", "prev_setting_name", "prev_weight", "prev_decay", "mean_mae", "std_mae",
