...
# 現行v1.4は0.20。まずは弱める方向も含めて比較します。
ELO_LAMBDA_WEIGHT_CANDIDATES = [
    0.00, 0.05, 0.10, 0.15, 0.20, 0.25,
]

OUTPUT_GRID_CSV = BASE_DIR / "v15_elo_weight_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v15_elo_weight_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v15_elo_weight_best_prediction.html"
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、PREV設定候補とElo補正強度を探索した結果です。
      大勝補正={GOAL_ADJUST_MODE}, cap={GOAL_CAP_FOR_STRENGTH},
...
def main():
    ...
    total = len(PREV_SETTING_CANDIDATES) * len(ELO_LAMBDA_WEIGHT_CANDIDATES)
    done = 0

    print("\n==============================")
    print("PREV設定候補 × Elo補正 グリッドサーチ開始")
    print("==============================")
    ...
    # 比較用: v1.5仮固定候補 PREV_WEIGHT=0.60, PREV_DECAY=0.985, Elo=0.20 との差分
    ...
    if len(baseline_rows) > 0:
        ...
        grid_df["delta_vs_prev060_decay0985_elo020"] = grid_df["mae"] - baseline_mae
    else:
        grid_df["delta_vs_prev060_decay0985_elo020"] = np.nan

    # 比較用: PREV_WEIGHT=0.40, PREV_DECAY=0.995, Elo=0.20 との差分
    alt_rows = grid_df[
        (grid_df["prev_setting_name"] == "prev040_decay0995")
        & np.isclose(grid_df["elo_lambda_weight"], 0.20)
    ]
    if len(alt_rows) > 0:
        alt_mae = float(alt_rows.iloc[0]["mae"])
        grid_df["delta_vs_prev040_decay0995_elo020"] = grid_df["mae"] - alt_mae
    else:
        grid_df["delta_vs_prev040_decay0995_elo020"] = np.nan
    ...
    show_cols = [
        ...
        "mae", "delta_vs_prev060_decay0985_elo020", "delta_vs_prev040_decay0995_elo020",
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        export_grid_html(
            ...
            title=(
                "J1 2025 順位予測 v1.5候補 "
                f"{best_prev_setting_name}, ELO={best_elo_lambda_weight:.2f}"
            ),
        )
