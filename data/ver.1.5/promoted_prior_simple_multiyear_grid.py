...
#   ・LAMBDA_CAP=3.5を固定し、昇格組の事前値補正を3年版で比較する
...
# 昇格組の事前値補正候補。
# none: 従来通り、前年J1データがないチームには前年補正を混ぜない。
# w010/w020: 昇格組だけ、前年成分の代わりに弱い事前値を少量混ぜる。
# 攻撃係数は1.00未満で攻撃弱め、守備係数は1.00超で守備弱めを表す。
PROMOTED_PRIOR_CANDIDATES = [
    {"name": "none", "promoted_prev_weight": 0.00, "promoted_attack_prior": 1.00, "promoted_defense_prior": 1.00},
    {"name": "w010_095_105", "promoted_prev_weight": 0.10, "promoted_attack_prior": 0.95, "promoted_defense_prior": 1.05},
    {"name": "w020_097_103", "promoted_prev_weight": 0.20, "promoted_attack_prior": 0.97, "promoted_defense_prior": 1.03},
    {"name": "w020_095_105", "promoted_prev_weight": 0.20, "promoted_attack_prior": 0.95, "promoted_defense_prior": 1.05},
]
...
# 昇格組補正: 前年J1にいないチームを検出し、候補ごとに事前値を混ぜる。
# none候補では promoted_prev_weight=0.0 なので従来通り補正なしになる。
...
# 出力
OUTPUT_DETAIL_CSV = BASE_DIR / "promoted_prior_simple_multiyear_detail.csv"
OUTPUT_SUMMARY_CSV = BASE_DIR / "promoted_prior_simple_multiyear_summary.csv"
OUTPUT_SUMMARY_HTML = BASE_DIR / "promoted_prior_simple_multiyear_summary.html"
OUTPUT_BEST_PREDICTION_CSV = BASE_DIR / "promoted_prior_simple_multiyear_best_predictions.csv"
...
def simulate_target_year(
    ...
    # Lambda Capは今回比較しない。前回検証で不採用のため1.00固定。
...
def export_summary_html(df, output_path):
    ...
    round_cols = [
        ...
        "max_match_draw_prob", "lambda_cap", "promoted_prev_weight", "promoted_attack_prior", "promoted_defense_prior",
        "matchup_time_decay", "mean_mae", "std_mae",
        "mean_prob_actual_position", "mean_sim_draw_rate", "promoted_mean_error",
        "mae_2023", "mae_2024", "mae_2025",
        "delta_vs_promoted_none",
    ]
...
<body>
  ...
  <div class="note">
    <p>
      ...
      ELO_LAMBDA_WEIGHT=0.20、大勝補正cap4、COMPAT_WEIGHT=0.20、DRAW_FACTOR=1.20、CURRENT_DECAY=1.000、LAMBDA_CAP=3.5を固定し、昇格組の事前値補正を比較しています。
    </p>
    <p>
...
def main():
    ...
    print("PROMOTED_PRIOR_CANDIDATES:", PROMOTED_PRIOR_CANDIDATES)
    ...
    total = len(PROMOTED_PRIOR_CANDIDATES) * len(TARGET_YEARS)
    done = 0
    ...
    for promoted_setting in PROMOTED_PRIOR_CANDIDATES:
        promoted_prior_name = str(promoted_setting["name"])
        promoted_prev_weight = float(promoted_setting["promoted_prev_weight"])
        promoted_attack_prior = float(promoted_setting["promoted_attack_prior"])
        promoted_defense_prior = float(promoted_setting["promoted_defense_prior"])
        ...
        for target_year in TARGET_YEARS:
            ...
            print(
                ...
                f"PROMOTED_PRIOR={promoted_prior_name}, "
                f"PROMOTED_W={promoted_prev_weight:.2f}, "
                f"PROMOTED_ATK={promoted_attack_prior:.2f}, "
                f"PROMOTED_DEF={promoted_defense_prior:.2f}, "
                f"CURRENT_DECAY_FIXED={current_decay:.3f}, "
            ...
            promoted_seed = (
                int(round(promoted_prev_weight * 10000))
                + int(round(promoted_attack_prior * 1000))
                + int(round(promoted_defense_prior * 1000))
            )
            seed_offset = (
    ...
    grouped = detail_df.groupby(
        ...
        mean_promoted_error=("promoted_mean_error", "mean"),
        mean_promoted_team_count=("promoted_team_count", "mean"),
    )
    ...
    # 比較基準: 昇格組補正なし、つまり従来設定
    base_rows = summary_df[summary_df["promoted_prior_name"].astype(str) == "none"]
    base_promoted_none = float(base_rows.iloc[0]["mean_mae"]) if len(base_rows) > 0 else np.nan
    summary_df["delta_vs_promoted_none"] = summary_df["mean_mae"] - base_promoted_none
    ...
    best_lambda_cap_label = str(summary_df.iloc[0]["lambda_cap_label"])
    best_promoted_prior_name = str(summary_df.iloc[0]["promoted_prior_name"])
    ...
    best_predictions_df = predictions_df[
        ...
        & (predictions_df["lambda_cap_label"] == best_lambda_cap_label)
        & (predictions_df["promoted_prior_name"] == best_promoted_prior_name)
    ...
    show_cols = [
        ...
        "goal_adjust_name", "compat_weight", "current_decay", "shrinkage", "draw_factor", "max_match_draw_prob_label", "lambda_cap_label",
        "promoted_prior_name", "promoted_prev_weight", "promoted_attack_prior", "promoted_defense_prior",
        "max_goals_for_score_grid", "matchup_prior_n", "matchup_time_decay",
        "mean_mae", "std_mae", "mean_prob_actual_position", "mean_sim_draw_rate", "mean_promoted_error",
        "mae_2023", "mae_2024", "mae_2025",
        "delta_vs_promoted_none",
    ]
    ...
    print("LAMBDA_CAP:", best_lambda_cap_label)
    print("PROMOTED_PRIOR:", best_promoted_prior_name)
