...
# v1.4のPREV_WEIGHT=0.4を中心に、低め〜高めまで再検証します。
PREV_WEIGHT_CANDIDATES = [
    0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30,
    0.35, 0.40, 0.45, 0.50, 0.55, 0.60,
]

OUTPUT_GRID_CSV = BASE_DIR / "v14_prev_weight_grid_results.csv"
OUTPUT_BEST_CSV = BASE_DIR / "v14_prev_weight_best_prediction.csv"
OUTPUT_BEST_HTML = BASE_DIR / "v14_prev_weight_best_prediction.html"


def simulate_with_prev_weight(
    prev_weight,
    n_sim,
    prepared,
    seed=42,
    ...
    """
    v1.4構造を保ったまま、PREV_WEIGHTだけを差し替えて順位予測する。
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    previous_strengths = prepared["previous_strengths"]
    ...
    for sim in range(1, n_sim + 1):
        ...
        if verbose and sim % 1000 == 0:
            print(f"  prev_weight={prev_weight:.2f}: {sim}/{n_sim} 回終了")
...
def export_grid_html(df, output_path, title):
    ...
    round_cols = [
        ...
        "position_error", "elo", "prev_weight_candidate", "effective_prev_weight"
    ]
    for col in round_cols:
        if col in display_df.columns:
...
<body>
  ...
  <div class="note">
    <p>
      v1.4をベースに、PREV_WEIGHTのみを再検証した結果です。
      大勝補正={GOAL_ADJUST_MODE}, cap={GOAL_CAP_FOR_STRENGTH},
...
def main():
    ...
    print("\n==============================")
    print("PREV_WEIGHT グリッドサーチ開始")
    print("==============================")

    for i, prev_weight in enumerate(PREV_WEIGHT_CANDIDATES, start=1):
        print(f"\n[{i}/{len(PREV_WEIGHT_CANDIDATES)}] PREV_WEIGHT={prev_weight:.2f}")
        summary, prediction_df = simulate_with_prev_weight(
            prev_weight=prev_weight,
            n_sim=N_SIM_SEARCH,
            prepared=prepared,
            seed=RANDOM_SEED,
    ...
    # v1.4既定値0.4との差分列を追加
    baseline_rows = grid_df[np.isclose(grid_df["prev_weight"], PREV_WEIGHT)]
    if len(baseline_rows) > 0:
        baseline_mae = float(baseline_rows.iloc[0]["mae"])
        grid_df["delta_vs_prev04"] = grid_df["mae"] - baseline_mae
    else:
        grid_df["delta_vs_prev04"] = np.nan
    ...
    if RUN_FINAL_BEST and best_summary is not None:
        ...
        final_summary, final_prediction = simulate_with_prev_weight(
        ...
        export_grid_html(
            final_prediction,
            OUTPUT_BEST_HTML,
            title=f"J1 2025 順位予測 v1.4 PREV_WEIGHT再検証 best={best_prev_weight:.2f}",
        )
        ...
        print(final_prediction[[
            ...
            "position_error", "prev_weight_candidate", "effective_prev_weight",
