...
# -*- coding: utf-8 -*-
"""
J1/J2統合Eloを、終了試合数別に比較検証するコード。

目的:
- Eloを導入しない beta_elo=0.00 を基準にする。
- 全チームが N 試合以上を終えた最初の日付をカットオフにする。
- 同じ日付の試合は前後に切り分けない。
- カットオフまでの実績順位表・得点失点からλを作る。
- 残り試合をポアソン分布でシミュレーションし、最終順位MAEを比較する。
...
実行例:
    python compare_elo_by_completed_matches.py ^
      ...
      --cutoff-matches-list 5,10,15,17,20 ^
...
class Config:
    ...
    outdir: str = "elo_by_completed_matches_outputs"

    target_division: str = "J1"
    years: str = "2023,2024,2025"
    cutoff_matches_list: str = "5,10,15,17,20"
    ...
    # cutoff到達判定
    # all_teams_at_least: 全チームがN試合以上を終えた最初の日付
    # average_team_at_least: 平均消化試合数がN以上になった最初の日付
    cutoff_mode: str = "all_teams_at_least"


def parse_args() -> Config:
    p = argparse.ArgumentParser(description="Elo比較・終了試合数別・同日非分割・ポアソン順位予測検証")
    ...
    p.add_argument("--cutoff-matches-list", default=Config.cutoff_matches_list, help="例: 5,10,15,17,20")
    ...
    p.add_argument("--cutoff-mode", choices=["all_teams_at_least", "average_team_at_least"], default=Config.cutoff_mode)
...
def table_games_by_team(matches: pd.DataFrame, teams: List[str]) -> Dict[str, int]:
...
def split_by_completed_matches_same_date(
    season: pd.DataFrame,
    teams: List[str],
    cutoff_matches: int,
    cutoff_mode: str = "all_teams_at_least",
    ...
    """
    同じ日付の試合を分断せず、終了試合数基準で known/future に分ける。

    all_teams_at_least:
        全チームの消化試合数が cutoff_matches 以上になった最初の日付までを known にする。
    average_team_at_least:
        平均消化試合数が cutoff_matches 以上になった最初の日付までを known にする。
    """
    season = season.sort_values(["date", "match_id"]).reset_index(drop=True)
    if len(season) == 0:
        raise ValueError("season が空です。")

    dates = sorted(season["date"].dropna().unique())
    chosen_date = None
    chosen_counts = None

    for d in dates:
        known_tmp = season[season["date"] <= d]
        counts = table_games_by_team(known_tmp, teams)
        min_games = min(counts.values())
        max_games = max(counts.values())
        avg_games = sum(counts.values()) / len(counts)
        if cutoff_mode == "all_teams_at_least":
            ok = min_games >= cutoff_matches
        elif cutoff_mode == "average_team_at_least":
            ok = avg_games >= cutoff_matches
        else:
            raise ValueError(f"未知の cutoff_mode です: {cutoff_mode}")
        if ok:
            chosen_date = pd.Timestamp(d)
            chosen_counts = counts
            break

    if chosen_date is None:
        raise ValueError(f"cutoff_matches={cutoff_matches} に到達する日付がありません。")

    known = season[season["date"] <= chosen_date].copy().reset_index(drop=True)
    future = season[season["date"] > chosen_date].copy().reset_index(drop=True)
    if chosen_counts is None:
        chosen_counts = table_games_by_team(known, teams)

    min_games = min(chosen_counts.values())
    max_games = max(chosen_counts.values())
    avg_games = sum(chosen_counts.values()) / len(chosen_counts)

    meta = {
        "cutoff_mode": cutoff_mode,
        "cutoff_matches": int(cutoff_matches),
        "cutoff_date": chosen_date.date().isoformat(),
        "known_matches": int(len(known)),
        "future_matches": int(len(future)),
        "known_ratio": float(len(known) / len(season)),
        "min_team_games_at_cutoff": int(min_games),
        "max_team_games_at_cutoff": int(max_games),
        "avg_team_games_at_cutoff": float(avg_games),
...
def estimate_strengths(current_matches: pd.DataFrame, prior_matches: pd.DataFrame, teams: List[str], cfg: Config) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    prev_stats, prev_league = weighted_stats(prior_matches, teams, cfg.goal_cap_for_strength, cfg.prev_decay)
    cur_strength = strengths_from_stats(cur_stats, cur_league)
    prev_strength = strengths_from_stats(prev_stats, prev_league)
...
def simulate_with_lambdas(
    ...
    for team, idx in team_to_idx.items():
        ...
        avg_pred = float(pos_sum[idx] / cfg.n_sims)
        detail_rows.append({
            ...
            "team": team,
            "actual_position": act,
            "avg_pred_position": avg_pred,
            "abs_error": float(abs(avg_pred - act)),
            "prob_actual_position": float(probs[act - 1]),
            "prob_top1": float(probs[0]),
            "prob_top3": float(probs[:3].sum()),
            "prob_bottom3": float(probs[-3:].sum()),
        })
    team_detail = pd.DataFrame(detail_rows).sort_values(["year", "cutoff_matches", "beta_elo", "avg_pred_position", "team"])
    ...
    metrics = {
        ...
        "actual_future_draw_rate": actual_draw_rate,
        "sim_goals_per_match": float(sim_goals / sim_total) if sim_total else float("nan"),
        "actual_future_goals_per_match": actual_gpm,
        "mean_lambda_home": float(lambdas["lambda_home"].mean()) if len(lambdas) else float("nan"),
        "mean_lambda_away": float(lambdas["lambda_away"].mean()) if len(lambdas) else float("nan"),
        "mean_lambda_total": float((lambdas["lambda_home"] + lambdas["lambda_away"]).mean()) if len(lambdas) else float("nan"),
        "mean_elo_factor": float(lambdas["elo_factor"].mean()) if len(lambdas) else float("nan"),
        "mean_abs_elo_diff": float(lambdas["elo_diff"].abs().mean()) if len(lambdas) else float("nan"),
    }
...
def run_one_cutoff(
    ...
    known, future, split_meta = split_by_completed_matches_same_date(
        season=season,
        teams=teams,
    ...
    if len(known) == 0 or len(future) == 0:
        raise ValueError(f"{year}年 cutoff={cutoff_matches} の分割結果が不正です: known={len(known)}, future={len(future)}")
    ...
    for beta in betas:
        ...
        lambdas["cutoff_matches"] = int(cutoff_matches)

        # 年・cutoff・betaごとに再現可能な乱数
        local_seed = int(cfg.seed + year * 100000 + cutoff_matches * 1000 + round(beta * 10000))
        rng = np.random.default_rng(local_seed)
        metrics, team_detail, distribution = simulate_with_lambdas(
            season=season,
            known=known,
            future=future,
            teams=teams,
        ...
        detail_rows.append({
            ...
            "cutoff_mode": cfg.cutoff_mode,
            "min_team_games_at_cutoff": split_meta["min_team_games_at_cutoff"],
            "max_team_games_at_cutoff": split_meta["max_team_games_at_cutoff"],
            "avg_team_games_at_cutoff": split_meta["avg_team_games_at_cutoff"],
        ...
        print(
            f"{year} cutoff={cutoff_matches:02d} beta={beta:.3f}: "
            f"known={len(known):3d}, future={len(future):3d}, "
...
def main() -> None:
    ...
    cutoff_list = parse_ints(cfg.cutoff_matches_list)
    ...
    for cutoff_matches in cutoff_list:
        for year in years:
            rows, team_dfs, dist_dfs, lambda_dfs = run_one_cutoff(
                year=year,
                cutoff_matches=cutoff_matches,
    ...
    detail = pd.DataFrame(all_detail_rows).sort_values(["cutoff_matches", "beta_elo", "year"]).reset_index(drop=True)

    summary = (
        detail.groupby(["cutoff_matches", "beta_elo"], as_index=False)
        .agg(
            n_years=("year", "count"),
            mean_mae=("mae", "mean"),
            std_mae=("mae", "std"),
            ...
            mean_actual_future_draw_rate=("actual_future_draw_rate", "mean"),
            mean_sim_goals_per_match=("sim_goals_per_match", "mean"),
            mean_actual_future_goals_per_match=("actual_future_goals_per_match", "mean"),
            mean_lambda_total=("mean_lambda_total", "mean"),
            mean_abs_elo_diff=("mean_abs_elo_diff", "mean"),
            mean_known_matches=("known_matches", "mean"),
            mean_future_matches=("future_matches", "mean"),
            mean_known_ratio=("known_ratio", "mean"),
            mean_min_team_games=("min_team_games_at_cutoff", "mean"),
            mean_max_team_games=("max_team_games_at_cutoff", "mean"),
        )
    ...
    # cutoffごとの最良beta
    best_by_cutoff = (
        summary.sort_values(["cutoff_matches", "mean_mae", "beta_elo"])
        .groupby("cutoff_matches", as_index=False)
        .first()
    )

    # beta=0との差分も付ける
    baseline = summary[summary["beta_elo"] == 0.0][["cutoff_matches", "mean_mae", "mean_prob_actual_position"]].rename(
        columns={"mean_mae": "baseline_mae", "mean_prob_actual_position": "baseline_prob_actual_position"}
    )
    summary = summary.merge(baseline, on="cutoff_matches", how="left")
    summary["mae_diff_vs_beta0"] = summary["mean_mae"] - summary["baseline_mae"]
    summary["prob_diff_vs_beta0"] = summary["mean_prob_actual_position"] - summary["baseline_prob_actual_position"]

    best_by_cutoff = best_by_cutoff.merge(baseline, on="cutoff_matches", how="left")
    best_by_cutoff["mae_diff_vs_beta0"] = best_by_cutoff["mean_mae"] - best_by_cutoff["baseline_mae"]
    best_by_cutoff["prob_diff_vs_beta0"] = best_by_cutoff["mean_prob_actual_position"] - best_by_cutoff["baseline_prob_actual_position"]

    team_detail = pd.concat(all_team_dfs, ignore_index=True) if all_team_dfs else pd.DataFrame()
    distribution = pd.concat(all_dist_dfs, ignore_index=True) if all_dist_dfs else pd.DataFrame()
    lambdas = pd.concat(all_lambda_dfs, ignore_index=True) if all_lambda_dfs else pd.DataFrame()

    config_df = pd.DataFrame([asdict(cfg) | {
        "parsed_years": ",".join(map(str, years)),
        "parsed_cutoff_matches_list": ",".join(map(str, cutoff_list)),
        "parsed_betas": ",".join(map(str, betas)),
    }])

    detail.to_csv(outdir / "elo_by_completed_matches_detail.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(outdir / "elo_by_completed_matches_summary.csv", index=False, encoding="utf-8-sig")
    best_by_cutoff.to_csv(outdir / "elo_by_completed_matches_best_by_cutoff.csv", index=False, encoding="utf-8-sig")
    team_detail.to_csv(outdir / "elo_by_completed_matches_team_detail.csv", index=False, encoding="utf-8-sig")
    distribution.to_csv(outdir / "elo_by_completed_matches_position_distribution.csv", index=False, encoding="utf-8-sig")
    lambdas.to_csv(outdir / "elo_by_completed_matches_lambdas.csv", index=False, encoding="utf-8-sig")
    config_df.to_csv(outdir / "elo_by_completed_matches_config.csv", index=False, encoding="utf-8-sig")

    print("\n=== best_by_cutoff ===")
    cols = ["cutoff_matches", "beta_elo", "mean_mae", "mae_diff_vs_beta0", "mean_prob_actual_position", "prob_diff_vs_beta0", "mean_known_matches", "mean_min_team_games", "mean_max_team_games"]
    print(best_by_cutoff[cols].to_string(index=False))
    print(f"\n出力先: {outdir.resolve()}")
